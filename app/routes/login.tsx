...
function validateInput(username: string, password: string) {
  ...
  if (username.length < 3) {
    res.un = "Usernames must be at least 3 characters long";
  }
  if (password.length < 3) {
    res.ps = "password must be at least 3 characters long";
  }
  return res;
}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const fieldErrors = validateInput(username, password);
  ...
  return createUserSession(user.id, "/blogs");
};
